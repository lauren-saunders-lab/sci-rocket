...
**Ignoring p5 or p7**: If all samples in your experiment have the same p5 or p7 barcode for, and want to simply ignore this barcode, then you can put "IGNORE" instead of e.g. "A01".
In this case sci-rocket will infer this barcode from the first read, assume it is the same for all reads, and the dashboard will not display QC info for this barcode.
